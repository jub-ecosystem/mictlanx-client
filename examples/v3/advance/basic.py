...
def simple_put(path:str):
    with open(path,"rb") as f:
        ...
        payload = PutPayload(key =nanoid_(),data = data,metadata = metadata)
...
def put_from_trace_proxy():
    auth_payload = AuthTokenPayload(secret="top_secret",expires_in = 7200)
    auth_service = Xolo(ip_addr = "localhost", port=10000, api_version=3)
    auth = auth_service.auth(auth_payload)
...
if __name__ == "__main__":
    ...
    c            = Client(rm_service = rm_service, auth_service = xolo,proxy = proxy_service,password = "t0p53cR3T#")
    with open("/source/01.pdf","rb") as f:
        ...
        payload = PutPayload(key="ball-0",data = data,metadata=metadata)
        res = c.put(payload)
        print(res)